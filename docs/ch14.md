...
> **Zod 官方文档**：[https://zod.dev/](https://zod.dev/) —— 完整 API 参考与更多示例。

---

## 本章练习

### 练习 1：定义 schema 并解析（重要程度 ⭐⭐⭐）

用 `z.object` 定义 `UserSchema`（`name: string`、`age: number`），用 `.parse()` 验证一个合法对象，并说明返回值的类型。
...
const user = UserSchema.parse({ name: "Alice", age: 25 });
user.name;  // 类型自动推断为 string
```
传入 `{ name: 42 }` 这样的非法数据会抛出 `ZodError`。
:::

### 练习 2：`safeParse`（重要程度 ⭐⭐⭐⭐）

不抛异常地验证数据：用 `safeParse` 检查 `{ id: "not-a-number" }` 是否符合 `z.object({ id: z.number() })`，并在失败分支打印错误信息。
...
if (!result.success) {
  ...
  console.log(result.data);  // 验证通过时才是安全数据
}
```
:::

### 练习 3：从 schema 推断类型（重要程度 ⭐⭐⭐）

定义 `PostSchema`（`id: number`、`title: string`），用 `z.infer` 得到 `Post` 类型并声明一个变量。
...
```
schema 与类型永远同步，改一处就全部生效。
:::

### 练习 4：字符串验证链（重要程度 ⭐⭐）

用链式方法定义一个"至少 3 个字符的邮箱"的 schema，并分别用合法与非法数据验证（体会 `parse` 抛错）。
...
EmailSchema.parse("a@b.com");   // OK
// EmailSchema.parse("x");      // Error：长度不足
// EmailSchema.parse("abc");    // Error：不是合法邮箱
```
链式顺序即验证顺序，先 `min(3)` 再 `email()`。
:::
