...
写代码时，我们经常需要告诉 TS"这个变量存的是什么"——是字符串、数字还是开关标志。这五个原始类型覆盖了 JS 中绝大多数基础数据：
...
> 💡 **实践**：`string`/`number`/`boolean` 日常最常用；`bigint` 用于超过 `2^53` 的整数；`symbol` 用于创建唯一的键。刚入门时记住前三个即可，后两个遇到时能认出来就行。
...
>
> 💡 **什么时候用**：`object` 的约束很宽——它只保证"这是一个对象"，不保证有哪些属性。实际工作中更常用的是**描述具体形状**的对象类型（见「类型别名」和第六章），`object` 主要用于"接受任意对象、但我不关心其结构"的通用函数参数。

## 数组

几乎所有程序都要处理"一组同类数据"——购物车里的商品、日志中的一行行记录。TS 用 `T[]` 描述"元素类型为 T 的数组"：
...
> 💡 **实践**：两种写法完全等价，`number[]` 更常用也更易读；`Array<number>` 在与泛型混写时（如 `Array<Array<number>>`）更清晰。注意 TS 的"数组"就是 JS 数组——元素可以随时 push/修改，`number[]` 只是编译时约束。
...
> 💡 **实践**：事件处理函数、`forEach` 回调、日志函数等"副作用型"函数，返回类型都标 `void` 就好。**不要**用 `undefined` 标注这类函数——`undefined` 暗示"返回值有意义但恰好是这个值"，而 `void` 明确说"没有返回值给你"。
...
> 枚举是 TypeScript 特有的功能——它不是纯类型层面的，编译后会生成真实的 JS 对象。官方文档建议"了解它存在，但除非确定需要，否则谨慎使用"。如果你只需要一组命名常量而不需要反向映射，可以用 `as const` 替代。更多细节见：[TypeScript 手册 · Enums](https://www.typescriptlang.org/docs/handbook/enums.html)
...
试着独立完成下面的练习（重要程度 ⭐ 越多越重要）。做完后再展开答案对照。

### 练习 1：类型注解基础（重要程度 ⭐⭐⭐）

给下面的变量和函数补全类型注解（`userName` 是字符串、`age` 是数字、`greet` 接收字符串返回字符串）：
...
写一个函数 `formatId(id: number | string): string`：传入数字时返回其字符串形式（用 `toString()`），传入字符串时原样返回。提示：用 `typeof` 收窄。
...
用 `type` 定义两个类型：`User`（对象，含必填的 `name: string`、`age: number` 和可选的 `email?: string`）和 `UserId`（`number | string`）。再用它们声明一个变量。
...
const id: UserId = 42;   // 数字或字符串都可以
```
:::

### 练习 4：`any` → `unknown`（重要程度 ⭐⭐⭐）

下面这个函数返回 `any`，使用方无法安全地使用返回值。请把它改成 `unknown`，并写出**安全使用**返回值的代码（提示：先判断是对象且含 `name` 属性）：
...
if (typeof data === "object" && data !== null && "name" in data) {
  const nm = (data as { name: unknown }).name; // 此时可安全访问
}
```
:::

### 练习 5：字面量类型与 `as const`（重要程度 ⭐⭐⭐⭐）

1. 定义字面量联合 `type Theme = "light" | "dark"`，并写一个 `setTheme(t: Theme): void` 函数；
2. 下面的配置对象没有 `as const` 时，`config.theme` 是什么类型？加上 `as const` 后呢？哪种写法能直接传给 `setTheme`？

```typescript
const config = { theme: "light", fontSize: 14 };
// config.theme 的类型是 ______（未加 as const）
```
...
const config = { theme: "light", fontSize: 14 };
// config.theme 的类型是 string（被拓宽了），不能直接传给 setTheme

const config2 = { theme: "light", fontSize: 14 } as const;
// config2.theme 的类型是 "light"（字面量），可以直接传：
setTheme(config2.theme);  // OK
```
:::

### 练习 6：用 `as const` 替代 `enum`（重要程度 ⭐⭐）

用 `as const` + 联合类型，定义一组 HTTP 方法常量 `GET`/`POST`/`PUT`/`DELETE` 及其字面量类型，并写一个只接受这四种方法的 `request(method)` 函数。
